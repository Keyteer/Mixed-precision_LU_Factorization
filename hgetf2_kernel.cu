...
__global__ void HGETF2_kernel(fp16 *panel, int ld, int rows, int cols, int *ipiv_panel) {
    ...
    for (int j = 0; j < cols; ++j) {
        ...
        // Use dynamic shared memory sizing based on actual block size
        extern __shared__ char shared_mem[];
        fp16 *max_vals = (fp16*)shared_mem;
        int *piv_indices = (int*)(shared_mem + blockDim.x * sizeof(fp16));
        ...
        if (tid + j < rows) {
            int row_idx = tid + j;
            fp16 val = __float2half(fabsf(__half2float(panel[j * ld + row_idx])));
            max_vals[tid] = val;
